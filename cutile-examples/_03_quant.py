...
def ct_quant(x: ct.Array, y: ct.Array, s: ct.Array, tile_size: ct.Constant):
    ...
    ct.store(x, (block_id, 0), tile_x)
    ct.store(s, (block_id, ), tile_s)

m, n = 1024, 1024
num_blocks, tile_size = m, n

x = torch.randn([m, n], device="cuda", dtype=torch.half) - 0.5
y = torch.empty_like(x)
scale = torch.empty([m, ], device="cuda", dtype=torch.float32)

ct.launch(torch.cuda.current_stream(), (num_blocks, ), ct_quant, (x, y, scale, tile_size))
