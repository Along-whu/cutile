...
def mla_splited_kernel(
    ...
    kv_block_num = ct.cdiv(cur_batch_seqlen, BLOCK_SIZE)
    ...
    for kv_block_idx in range(kv_block_start_idx.item(), kv_block_end_idx.item()):
        ...
        physical_kv_block_idx = physical_kv_block_idx.item()
        
        # [BLOCK_SIZE, KV_LORA_RANK]
        k_nope_tile = ct.load(k_nope, (physical_kv_block_idx * BLOCK_SIZE, 0), (BLOCK_SIZE, KV_LORA_RANK), padding_mode=ct.PaddingMode.ZERO).astype(ct.float32)
        # [BLOCK_SIZE, ROPE_DIM]
        k_rope_tile = ct.load(k_rope, (physical_kv_block_idx * BLOCK_SIZE, 0), (BLOCK_SIZE, ROPE_DIM), padding_mode=ct.PaddingMode.ZERO).astype(ct.float32)
        
        # qk: [TILE_H, BLOCK_SIZE]
        ...
        mask_n = kv_block_idx * BLOCK_SIZE + ct.arange(BLOCK_SIZE, dtype=ct.int32) < cur_batch_seqlen # [BLOCK_SIZE]
        qk = ct.where(mask_n.reshape((1, BLOCK_SIZE)), qk, float("-inf"))
...
def merging_attention_states_kernel(
    ...
    for split_idx in range(N_SPLITS):
        ...
        if start < end: # pass empty splits
